...
| 07    | Affiliate | zone_a | Shop Pickup   | Express  | Mobile Money Ref | PARTIAL PASS | ORD-1773552582512 created fresh; affiliate and customer payment attempts to /v1/payments/mobile-money returned 403 Forbidden; admin workaround succeeded with ref MM-ADMIN-07-20260315083219, paymentId=1a6de74c-a587-4f72-b609-40d34c2f37e4, receipt=RCP-20260315-7276; timeline now shows PAID by ADMIN; affiliate mobile-money permission defect remains open |
| 08    | Affiliate | zone_a | Hybrid Return | Standard | Cash             | BLOCKED      | Affiliate hybrid-return create failed during fresh rerun: Affiliate hybrid-return create failed with 400 Bad Request using returnMethod=DELIVER_TO_DOOR                                                                                                                                                                                                          |
...
| DR-09 | Affiliate/customer mobile-money endpoint returns 403 while admin succeeds | ORD-1773552582512 | P1 | Admin | Record payment as admin temporarily | Update authorization policy for /v1/payments/mobile-money so intended non-admin operator roles can record valid order payments |
...
- PASS / FAIL: FAIL
- Summary: Orders 01-07 were exercised with varying levels of proof. Orders 08 and 09 are blocked on affiliate hybrid-return create with 400 Bad Request. Order 07 mobile-money is only recordable by admin, while affiliate/customer attempts return 403. Order 10 is blocked because no QC fail/resolve route could be executed from the available scripted/admin candidates.
- Follow-up actions: Keep the dry-run marked FAIL. Fix affiliate hybrid-return create for orders 08 and 09, fix non-admin payment authorization for mobile-money and cash recording, and expose or document explicit QC fail/resolve endpoints so Order 10 can be completed.
