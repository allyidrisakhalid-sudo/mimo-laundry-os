...
Set `DATABASE_URL` in `apps/api/.env` for local development.
...
Prisma 7 reads the database URL from `apps/api/prisma.config.ts`.
The schema file remains at `apps/api/prisma/schema.prisma`.
...
- Prisma config lives at `apps/api/prisma.config.ts`
- Migration artifacts live at `apps/api/prisma/migrations`
